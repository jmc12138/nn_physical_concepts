...
class Network(object):
        ...
        with self.graph.as_default():
            ...
            for epoch_iter in tqdm(range(epoch_num)):
                ...
                for step, data_dict in enumerate(self.gen_batch(training_data, batch_size)):
                    ...
                    feed_dict = mydict(data_dict, parameter_dict)
    ...
    def test(self, data, beta=0):
        ...
        with self.graph.as_default():
            ...
            summary = self.session.run(self.all_summaries, feed_dict=mydict(data_dict, parameter_dict))
    ...
    def save(self, file_name):
        ...
        with self.graph.as_default():
            ...
            saver.save(self.session, os.path.join(io.tf_save_path,file_name) + '.ckpt')
            params = {'latent_size': self.latent_size,
                      ...
                      'name': self.name}
            with open(os.path.join(io.tf_save_path,file_name) + '.pkl', 'wb') as f:
    ...
    def from_saved(cls, file_name, change_params={}):
        ...
        """
        with open(os.path.join(io.tf_save_path,file_name) + '.pkl', 'rb') as f:
    ...
    def graph_setup(self):
        ...
        with self.graph.as_default():
            ...
            self.summary_writer = tf.summary.FileWriter(os.path.join(io.tf_log_path,self.name) + '/', graph=self.graph)
    ...
    def load(self, file_name):
        ...
        with self.graph.as_default():
            saver = tf.train.Saver()
            saver.restore(self.session, os.ppath.join(io.tf_save_path,file_name) + '.ckpt')
...
if __name__ == '__main__':
    analysis_path = r'F:\demo\zyb\nn_physical_concepts - 副本\analysis\qubit_example.ipynb'
    ne = Network(100,100)
    print("Now is well")
